...
const DepartmentEventDetails = ({ departmentId }: DepartmentEventDetailsProps) => {
  ...
  const isAutoExpo = department.id === "auto-expo";
  ...
  return (
    <div className="w-full h-full flex items-center justify-center overflow-hidden z-20 pt-30 md:pt-40">
      ...
      <div className="relative z-10 max-w-xs md:max-w-6xl md:mx-auto px-4 md:px-4 py-4 md:py-8 dark:bg-black/50 rounded-2xl md:rounded-3xl shadow-xl md:shadow-2xl border border-white/10 dark:border-white/20 bg-white/50">
        ...
        {/* Auto Expo Special Layout or Regular Events Table */}
        {isAutoExpo ? (
          <div className="space-y-6 md:space-y-8 p-4 md:p-6 rounded-lg md:rounded-xl border border-white/10 dark:border-white/20 bg-white/40 dark:bg-black/40 shadow-md md:shadow-lg">
            <div className="space-y-4">
              <h3 className="text-2xl md:text-3xl font-bold text-black dark:text-white text-center font-poppins">
                {department.name}
              </h3>
              <p className="text-sm md:text-xl font-semibold text-purple-600 dark:text-purple-400 text-center">
                {department.details}
              </p>
            </div>
          </div>
                    ...
                    <Popover>
                      ...
                      <PopoverContent className="bg-white/80 dark:bg-black/80 backdrop-blur-sm border border-white/20 text-xs md:text-sm">
                        <div className="space-y-1 md:space-y-2">
                          ...
                          <p className="text-black dark:text-white"><span className="text-yellow-600 dark:text-yellow-400">Non-Technical/Technical</span> - Events that allow both technical and non-technical Projects</p>
                          <p className="text-black dark:text-white"><span className="text-purple-600 dark:text-purple-400">Formal</span> - Events that are more structured and require formal attire</p>
                    ...
                    <td className="p-3 md:p-4 border-l border-white/10 dark:border-white/20">
                      <span className={cn(
                        "px-2 md:px-3 py-0.5 md:py-1 rounded-full text-[10px] md:text-xs font-medium transition-colors",
                        {
                          ...
                          "bg-yellow-500/10 text-yellow-700 dark:text-yellow-400 group-hover:bg-yellow-500/20": event.type === "Non-Technical/Technical",
                          "bg-purple-500/10 text-purple-700 dark:text-purple-400 group-hover:bg-purple-500/20": event.type === "Formal",
